...
var CM_ASSETS = [
	...
	{ css: 'dae/lib/codemirror.min.css' },
	{ css: 'dae/addon/fold/foldgutter.min.css' },
	{ css: 'dae/theme/dracula.min.css' },
	{ js: 'dae/lib/codemirror.min.js' },
	{ js: 'dae/addon/edit/matchbrackets.min.js' },
	{ js: 'dae/addon/edit/closebrackets.min.js' },
	{ js: 'dae/addon/selection/active-line.min.js' },
	{ js: 'dae/addon/fold/foldcode.min.js' },
	{ js: 'dae/addon/fold/foldgutter.min.js' },
	{ js: 'dae/addon/fold/indent-fold.min.js' },
	{ js: 'dae/mode/dae/dae.js' }
];

var CM_STYLE = [
	'.dae-cm .CodeMirror { margin-right: 200px; border: 1px solid #6272a4; border-radius: 6px; height: auto;',
	'	min-height: 400px; font-family: "Fira Code", "Monaco", "Consolas", monospace;',
	'	font-size: 13px; box-shadow: 0 4px 6px rgba(0,0,0,0.3); }',
	'.dae-cm .cm-s-dracula.CodeMirror { background-color: #282a36 !important; color: #f8f8f2 !important; }',
	'.dae-cm .cm-s-dracula .CodeMirror-gutters { border-right: none !important; background-color: #282a36 !important; }',
	'.dae-cm .cm-s-dracula .CodeMirror-linenumber { color: #6272a4 !important; }',
	'.dae-cm .cm-s-dracula .cm-keyword, .dae-cm .cm-s-dracula .cm-operator { color: #ff79c6 !important; font-weight: bold; }',
	'.dae-cm .cm-s-dracula .cm-variable-3 { color: #ffb86c !important; }',
	'.dae-cm .cm-s-dracula .cm-def { color: #50fa7b !important; }',
	'.dae-cm .cm-s-dracula .cm-number { color: #bd93f9 !important; }',
	'.dae-cm .cm-s-dracula .cm-string { color: #f1fa8c !important; }',
	'.dae-cm .cm-s-dracula .cm-comment { color: #6272a4 !important; font-style: italic; }',
	/* 手机端（<768px）：标签堆到上方、字段占满宽度，编辑器去掉右侧 200px 边距 */
	'@media (max-width: 767px) {',
	'	.dae-cm .cbi-value { flex-direction: column; align-items: stretch; }',
	'	.dae-cm .cbi-value-title { flex: none; text-align: left; padding: 0 0 6px 0; }',
	'	.dae-cm .cbi-value-field { margin-left: 0; }',
	'	.dae-cm .CodeMirror { margin-right: 0; min-height: 320px; }',
	'}'
].join('\n');
...
function ensureCmStyles(node) {
	...
	if (!document.getElementById('dae-cm-style'))
		styleHost(node).appendChild(E('style', { id: 'dae-cm-style' }, CM_STYLE));
...
function statusCard() {
	...
	function refresh() {
		return fs.exec_direct('/usr/libexec/dae-status').then(function(stdout) {
...
function editorPage(opts) {
	...
	return view.extend({
		render: function() {
			var m = new form.Map('dae', opts.title, opts.description);

			if (opts.uciSection) {
				var st = m.section(form.TypedSection, 'dae');
				st.anonymous = true;
				opts.uciSection(st);
			}
			...
			o.write = function(section_id, value) {
				return fs.write(path, value, 416 /* 0640，与 dae 启动脚本收紧后的权限一致 */);
			};
			...
			return m.render().then(L.bind(function(node) {
				...
				return E('div', { 'class': 'dae-cm' }, node);
			}, this));
		},
