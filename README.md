...
OpenWrt 上 [dae](https://github.com/daeuniverse/dae)（eBPF 透明代理）的自建打包仓库，只产出 **apk**（OpenWrt 25.x apk 体系）。

## 构成

| 部分 | 说明 |
| --- | --- |
| 核心包 `dae` | 直接取上游 Release 的预编译静态产物（`dae-linux-x86_64_v3_avx2.tar.xz` / `dae-linux-arm64.tar.xz` 内的 `usr/bin/dae`，静态链接、musl 可直接运行） |
...
| `update-dae` 工作流 | 每日检查上游 release，自动 bump `PKG_VERSION` / `PKG_RELEASE` 并触发构建 |
...
# 需先从 Actions 产物或上游 release.yml 产物获取对应二进制并放好：
...
**AGPL-3.0-only**（与 `dae/Makefile` 的 `PKG_LICENSE` 一致）。上游 dae 源码为 AGPL-3.0（daeuniverse/dae）；LuCI 界面移植自 QiuSimons/luci-app-dae。

## 鸣谢

- [daeuniverse/dae](https://github.com/daeuniverse/dae) 及其贡献者（dae 引擎，本仓库二进制来源）；[QiuSimons/luci-app-dae](https://github.com/QiuSimons/luci-app-dae)（界面行为与文案的移植来源）；
