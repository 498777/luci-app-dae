...
- 二进制：直接取 [daeuniverse/dae](https://github.com/daeuniverse/dae) **release** 的预编译产物（`dae-linux-x86_64_v3_avx2.tar.xz` / `dae-linux-arm64.tar.xz` 内的 `usr/bin/dae`，静态链接、musl 可直接运行）；版本跟随上游 release（`PKG_VERSION` = 上游 tag 去掉 `v`），由 `update-dae` 每日检查并自动同步、触发构建；
...
- **geo 数据**：依赖官方 `v2ray-geoip`/`v2ray-geosite`；安装脚本会自动在 `/usr/share/dae` 建立软链（无需手动）；
...
- `geoip:`/`geosite:` 所需数据由 `v2ray-geoip`/`v2ray-geosite` 依赖带入；安装脚本自动在 `/usr/share/dae` 建软链；
...
- **x86_64v3**：官方 OpenWrt / ImmortalWrt 无 `x86_64_v3` 架构/SDK，v3 只是同一 amd64 的
  GOAMD64 档位（上游亦如此）。本仓库包架构为 `x86_64`、二进制按 v3 编译，需要 CPU
...
1. **取核心二进制**：从上游 release 下载预编译静态 `dae`
   （x86_64 取 `dae-linux-x86_64_v3_avx2.tar.xz`，aarch64 取 `dae-linux-arm64.tar.xz`），
   解出 `usr/bin/dae` 放入 `dae/files/prebuilt/<arch>/dae`；
   dae 核心**不在 OpenWrt SDK 内编译**（上游的 eBPF 生成与 SDK 的 bpf-headers 不兼容）。
2. **打包 job**：OpenWrt SDK 只负责把预编译二进制装进 `dae` 包并编 luci/语言包。

推送 `main`，或在 **Actions → Build apk → Run workflow** 手动触发（SDK 默认
`openwrt-25.12`）。Release 使用当天日期槽位 `dae_<日期>`，包版本为 `dae-<上游版本>-rN`（上游 tag 去掉 `v`，如同步 `v2.1.1` 则包版本为 `dae-2.1.1-r1`；apk 要求版本以数字开头）：
...
**AGPL-3.0-only**（与 `dae/Makefile` 的 `PKG_LICENSE` 一致）。上游 dae 源码 AGPL-3.0（daeuniverse/dae）；LuCI 界面移植自 QiuSimons/luci-app-dae。

## 鸣谢

- [daeuniverse/dae](https://github.com/daeuniverse/dae) 及其贡献者（dae 引擎，本仓库二进制来源）；[QiuSimons/luci-app-dae](https://github.com/QiuSimons/luci-app-dae)（LuCI 界面移植来源）；
