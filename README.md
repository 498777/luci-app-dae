...
- 二进制：从 [olicesx/dae](https://github.com/olicesx/dae) **kdae** 分支预编译（跟随上游最新提交；版本号取上游 commit 前 7 位（加 `2.0.0.` 前缀以符合 apk 版本规则），由 `update-dae` 每日检查并自动同步、触发构建）；
...
`openwrt-25.12`）。Release 使用当天日期槽位 `dae_<日期>`，包版本为 `dae-2.0.0.<上游 commit 前 7 位>-rN`（apk 要求版本以数字开头，故加 `2.0.0.` 前缀；如 `dae-2.0.0.e11f194-r1`）：
